#!/usr/bin/env python3
"""Unified Log P-Spline MCMC runner for LISA TDI noise datasets.

Usage
-----
  python run_mcmc.py --dataset noise_4a --model multivar
  python run_mcmc.py --dataset noise_5a --model univar --duration-days 30
  python run_mcmc.py --dataset noise_4a --model multivar --eta 0.05 --compute-lnz
  python run_mcmc.py --dataset noise_5a --model multivar --outdir /path/to/custom
"""
...
from run_univar_aet import run_univar_aet_analysis, xyz_to_aet_timeseries, xyz_to_aet_matrix
...
class DatasetConfig:
    data_path: Path
    outdir_base: Path
    reference: str       # "analytic" | "welch"
    theta_im_knots: int


DATASET_CONFIGS: dict[str, DatasetConfig] = {
    "noise_4a": DatasetConfig(
        data_path=Path("noise_4a/tdi.h5"),
        outdir_base=Path("noise_4a"),
        reference="analytic",
        theta_im_knots=2,
    ),
    "noise_5a": DatasetConfig(
        data_path=Path("noise_5a/tdi.h5"),
        outdir_base=Path("noise_5a"),
        reference="segwo",
        theta_im_knots=50,
    ),
}
...
class ModelSpec:
    ...
    k_theta: int | None  # theta knot count (None = not applicable)
    outdir_name: str   # sub-directory name under <dataset>/
...
# Near-diagonal AET model: very few off-diagonal knots to allow negligible
# but non-zero cross-correlations (effectively forces near-zero thetas).
...
def _posterior_coherence_from_sd_quantiles(
    sd_quantiles: np.ndarray,
) -> np.ndarray:
    """Build coherence quantiles from spectral-density quantiles.

    The coherence definition used here is
    |S_ij(f)| / sqrt(S_ii(f) * S_jj(f)),
    clipped to [0, 1] and sorted across quantile axis to preserve ordering.
    """
    if sd_quantiles.ndim != 4 or sd_quantiles.shape[0] != 3:
        raise ValueError(
            "sd_quantiles must have shape (3, n_freq, 3, 3); "
            f"got {sd_quantiles.shape}."
        )
    ...
    for i in range(3):
        for j in range(3):
            ...
            if i == j:
                ...
                emp_psd = np.maximum(emp_diag[i].real, 1e-30)
                ax.loglog(freq_plot, ref_psd, label=f"{reference_label} PSD")
                ax.loglog(freq_plot, emp_psd, alpha=0.5, label=f"{empirical_label} PSD")
                if post_real is not None:
                    q05 = np.maximum(post_real[0, :, i, i], 1e-30)
                    q50 = np.maximum(post_real[1, :, i, i], 1e-30)
                    q95 = np.maximum(post_real[2, :, i, i], 1e-30)
                    ax.fill_between(freq_plot, q05, q95, alpha=0.2, color="C2")
                    ax.loglog(freq_plot, q50, color="C2", lw=1.5, label="Posterior median")
            ...
            if post_coh is not None:
                q05 = np.clip(post_coh[0, :, i, j], 0.0, 1.0)
                q50 = np.clip(post_coh[1, :, i, j], 0.0, 1.0)
                q95 = np.clip(post_coh[2, :, i, j], 0.0, 1.0)
                ax.fill_between(freq_plot, q05, q95, alpha=0.2, color="C2")
                ax.semilogx(freq_plot, q50, color="C2", lw=1.5, label="Posterior median")
            ax.set_ylim(0, 1.05)
    ...
    *,
    ...
    for i in range(3):
        for j in range(3):
            ...
            ax.semilogx(
                freq_plot, component(emp_csd), alpha=0.55,
                label=f"{empirical_label} {label}",
            )
            if post_real is not None and post_imag is not None:
                ...
                ax.fill_between(freq_plot, q05, q95, alpha=0.2, color="C2")
                ax.semilogx(freq_plot, q50, color="C2", lw=1.5, label="Posterior median")
                _set_symlog_scale(ax, [component(ref_csd), component(emp_csd), q05, q95])
            else:
...
def build_plot_inputs(
    ...
    if nperseg < 1:
        raise ValueError(
            f"Need at least {plot_blocks} samples for {plot_blocks} Welch blocks; got {n_trim}."
        )
    noverlap = int(DIAG_WELCH_OVERLAP_FRACTION * nperseg)
    noverlap = min(noverlap, max(0, nperseg - 1))
    fs = 1.0 / lisa.delta_t
    x, y, z = lisa.data[:n_trim].T
    kwargs = {
        "fs": fs,
        "window": WISHART_WINDOW,
        "nperseg": nperseg,
        "noverlap": noverlap,
        "detrend": False,
    }
    ...
    s_emp = {
        "Sxx": Sxx[LOW_FREQ_BIN_TRIM:],
        "Syy": Syy[LOW_FREQ_BIN_TRIM:],
        "Szz": Szz[LOW_FREQ_BIN_TRIM:],
        "Sxy": Sxy[LOW_FREQ_BIN_TRIM:],
        "Syz": Syz[LOW_FREQ_BIN_TRIM:],
        "Szx": Szx[LOW_FREQ_BIN_TRIM:],
    }
    overlap_pct = int(round(100.0 * noverlap / nperseg)) if nperseg > 0 else 0
    empirical_label = f"Welch ({plot_blocks} blocks, {overlap_pct}% overlap)"
    return freq, true_cov, s_emp, empirical_label
...
def make_block_welch_plots(
    ...
    *,
    ...
    psd_plot = outdir / "welch_psd_triangle.png"
    csd_plot = outdir / "welch_psd_csd_matrix.png"
    plot_psd_triangle(
        freq, true_cov, s_emp, psd_plot,
        empirical_label=empirical_label,
        reference_label="Reference",
        posterior_quantiles=posterior_quantiles,
    )
    plot_psd_csd_matrix(
        freq, true_cov, s_emp, csd_plot,
        reference_label="Reference",
        empirical_label=empirical_label,
        posterior_quantiles=posterior_quantiles,
    )
    print(f"Saved PSD/coherence plot to {psd_plot.resolve()}")
    print(f"Saved PSD/CSD plot to {csd_plot.resolve()}")
...
def build_multivar_config(
    ...
    exclude_bands = light_travel_null_exclusion_bands(
        ...
        halfwidth=NULL_EXCISION_HALFWIDTH,
    )
    print(f"Using null-band exclusions: {exclude_bands}")
    n_knots = {
...
def _prepare_run(
    ...
    """Resolve and validate shared run parameters; return (eta, duration_days, duration_label, nb, n_trim, main_nc, vi_nc)."""
...
def run_multivar(args: argparse.Namespace, cfg: DatasetConfig, lisa: LISAData, *, outdir_name: str = "mcmc_output_H4") -> None:
    eta, duration_days, duration_label, nb, n_trim, main_nc, vi_nc = _prepare_run(
        args, cfg, lisa
    )
    eta_label = f"eta{eta:g}"
    outdir = args.outdir or (cfg.outdir_base / outdir_name / duration_label / eta_label)
    outdir.mkdir(parents=True, exist_ok=True)

    numpyro.set_host_device_count(NUM_CHAINS)

    print(
        f"Using n_trim={n_trim} samples, "
        f"Nb={nb} Wishart blocks (~{duration_days / nb:.2f} days per block)."
    )
    ...
    print(
        f"Band [{FMIN:.1e}, {FMAX:.1e}] Hz: "
        f"main_nc={main_nc}, vi_nc={vi_nc}, "
        f"coarse factor ~ {coarse_factor:.3f}, eta={eta:.6g}, "
        f"alpha_delta=beta_delta={ALPHA_DELTA:.3g}, "
        f"wishart_floor_fraction={WISHART_FLOOR_FRACTION:.1e}."
    )
    ...
    config = build_multivar_config(
        ...
        lnz_n_resamples=int(args.lnz_n_resamples),
        lnz_n_estimations=int(args.lnz_n_estimations),
        lnz_max_iter=int(args.lnz_max_iter),
        lnz_tol=float(args.lnz_tol),
        lnz_kde_bw=str(args.lnz_kde_bw),
    )

    pipeline = make_pipeline(ts, config)
    result = pipeline.run()
    true_psd = align_true_psd_to_freq(config.true_psd, pipeline.data)
    result.save(str(outdir), true_psd=true_psd)
    idata = result.idata
    ...
    if bool(args.compute_lnz):
        print(
            "lnZ summary:",
            {
                "lnz": idata.attrs.get("lnz"),
                "lnz_err": idata.attrs.get("lnz_err"),
                "lnz_valid": idata.attrs.get("lnz_valid"),
            },
        )

    print("MCMC sampling complete, plotting results...")
...
def run_multivar_aet(args: argparse.Namespace, cfg: DatasetConfig, lisa: LISAData) -> None:
    """Run the multivar model in AET basis.

    Uses identical Wishart likelihood infrastructure as multivar(XYZ).
    --theta-knots controls off-diagonal spline flexibility:
      k=50  → H2: full AET covariance (same capacity as XYZ model)
      k=1   → H1: effectively diagonal AET (one near-zero constant per pair)
    Compare lnZ against univar(AET) H0 to test whether AET off-diagonals matter.
    """
    eta, duration_days, duration_label, nb, n_trim, main_nc, vi_nc = _prepare_run(
        args, cfg, lisa
    )
    k_theta = args.theta_knots if args.theta_knots is not None else N_SPLINE_KNOTS_THETA_AET
    eta_label = f"eta{eta:g}"
    _outdir_name = getattr(args, "_outdir_name", f"mcmc_output_multivar_aet_k{k_theta}")
    outdir = args.outdir or (
        cfg.outdir_base / _outdir_name / duration_label / eta_label
    )
    outdir.mkdir(parents=True, exist_ok=True)
    ...
    print(
        f"Running multivar(AET) for {args.dataset}: "
        f"duration={duration_days:g}d, Nb={nb}, eta={eta:.6g}, "
        f"theta_re/im knots={k_theta}."
    )

    # Transform XYZ time series → AET
    ...
    # Rotate true PSD reference matrix to AET basis for plotting overlays
    ...
    config = build_multivar_config(
        cfg=cfg,
        true_psd_freq=lisa.freq,
        true_psd_matrix=true_psd_matrix_aet,
        nb=nb,
        main_nc=main_nc,
        vi_nc=vi_nc,
        eta=eta,
        ...
        lnz_tol=float(args.lnz_tol),
        lnz_kde_bw=str(args.lnz_kde_bw),
        theta_re_knots=k_theta,
        theta_im_knots=k_theta,
    )
...
def run_univar(args: argparse.Namespace, cfg: DatasetConfig, lisa: LISAData, *, outdir_name: str = "mcmc_output_H0") -> None:
    eta, duration_days, duration_label, nb, n_trim, main_nc, vi_nc = _prepare_run(
        args, cfg, lisa
    )
    eta_label = f"eta{eta:g}"
    root_outdir = args.outdir or (
        cfg.outdir_base / outdir_name / duration_label / eta_label
    )
    numpyro.set_host_device_count(NUM_CHAINS)
    ...
    print(
        f"Running {args.dataset} univariate AET analysis: "
        f"duration={duration_days:g}d, Nb={nb}, eta={eta:.6g}, "
        f"main_nc={main_nc}, vi_nc={vi_nc}."
    )
    print(f"Using null-band exclusions: {exclude_bands}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Log P-Spline MCMC on LISA TDI noise data.",
    )
    parser.add_argument(
        "--dataset",
        required=True,
    ...
    _model_help = "  |  ".join(
        f"{key}: {spec.description}" for key, spec in MODEL_SPECS.items()
    )
    parser.add_argument(
        "--model",
        required=True,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Duration in days. Defaults to the dataset default.",
    )
    parser.add_argument(
        "--eta",
        type=float,
        default=None,
        help="Posterior tempering weight in (0, 1]. Defaults to the dataset default.",
    )
    parser.add_argument(
        "--outdir",
        type=Path,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=COMPUTE_LNZ,
        help="Run MorphZ lnZ estimation after sampling.",
    )
...
def main() -> None:
    ...
    if spec.runner == "multivar":
        ...
        args.theta_knots = spec.k_theta
        args._outdir_name = spec.outdir_name
        run_multivar_aet(args, cfg, lisa)
    else:
